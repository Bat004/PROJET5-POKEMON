import './style.css'
import './carte.ts'
import { genererSquelette, mettreAJourPagination, creerCarte } from './ui.ts'
import { initialiserRecherche, filtrerLesPokemons } from './search.ts'
import { initialiserPagination, obtenirPageActuelle } from './pagination.ts'
        ...
        if (liste) {
            ...
            for (const p of PokemonAAfficher) {
                ...
                const image = info.sprites.other['official-artwork'].front_default
                    liste.innerHTML += creerCarte(info.name, image, p.url, info.id, info.cries.latest)
                }
        }
    } catch (e) {
        console.error(e)
